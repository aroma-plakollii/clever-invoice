...
import { settingGetSingle } from "@/app/api/settingService";
import { Setting, SettingError } from "@/app/types/setting";
import { SettingUpdateButtons } from "@/app/components/settingComponents/SettingUpdateButtons";
...
interface SettingUpdateFormProps{
    idSetting: string | string[];
}

export const SettingUpdateForm: React.FC<SettingUpdateFormProps> = (props) => {
    const headers = useHeaders();
    const [setting, setSetting] = useState<Setting>();
    const [image, setImage] = useState<File | null | string>(setting?.image);
    const [imageUrl, setImageUrl] = useState<string | null>(null);
    const [loading, setLoading] = useState<boolean>(true);
    const [inputFields, setInputFields] = useState([{ label: "", value: "" }]);
    const [inputFieldsErrors, setInputFieldsErrors] = useState<any[]>([{ label: false, value: false }]);
    ...
    const id = Number(props.idSetting);
    ...
    const compressImage = useCallback(async (base64Image: string) => {
        try {
    ...
    useEffect(() => {
        const getSetting = async () => {
            const settingGetSingleResponse = await settingGetSingle(id, headers);

            if (settingGetSingleResponse) {
                setSetting(settingGetSingleResponse.setting);
                setLoading(false);
            }

            await compressImage(settingGetSingleResponse.setting?.image);
            const base64Image = settingGetSingleResponse.setting?.image;
            if (base64Image) {
                if (!base64Image.startsWith("data:image")) {
                    const formattedImage = `data:image/png;base64,${base64Image}`;
                    setImageUrl(formattedImage);
                } else {
                    setImageUrl(base64Image);
                }
            }

            if (settingGetSingleResponse.setting?.customFields) {
                const customFieldsEntries = Object.entries(settingGetSingleResponse.setting.customFields);
                if (customFieldsEntries.length > 0) {
                    const detailsArray = customFieldsEntries.map(([key, value]) => ({
                        label: key,
                        value: String(value)
                    }));
                    setInputFields(detailsArray);
                }
            }
        };

        getSetting();
    }, [headers, id, compressImage]);
            ...
            {loading ? <DashboardFormLoader type={'full'} /> : (
                    <div className="flex items-start">
                        <form action="#" method="POST"
                              className="mx-auto mt-2 w-full py-7 px-5 xl:p-7 shadow-2xl rounded-2xl">
                            <h1 className="text-xl font-bold mb-5">Edit Setting</h1>
                            <div className="grid gap-x-3 gap-y-6 grid-cols-7">
                                <div
                                    ...
                                    <div className="col-span-2">
                                        <div className="flex items-center justify-center w-full">
                                            <label
                                                ...
                                                className="relative flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
                                                style={{
                                ...
                                <div className="col-span-7">
                                    <SettingUpdateButtons setLoading={setLoading} setting={setting} image={image} errors={errors}
                                                          setErrors={setErrors}
                                                          headers={headers} inputFields={inputFields}
